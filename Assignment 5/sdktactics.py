        ...
        for group in groups:
            ...
            for tile in group:
                if tile.symbol != sdkboard.OPEN:
                    if tile.symbol in result:
                        copy.add(tile.symbol)
                        duplicate = False
                    else:
                        result.add(tile.symbol)
            for other in group:
                if other.symbol in copy:
                    other.announce("duplicate")                   
        return duplicate
...
def hidden_single(group):
        ...
        """
        for digit in sdkboard.DIGITS:
            num_tiles = 0
            cur_tile = 0
            for tile in group:
                if digit in tile.possible and tile.symbol == sdkboard.OPEN:
                    cur_tile = tile
                    num_tiles += 1
            if num_tiles == 1:
                cur_tile.determine(digit)
        return
