...
def vwall(cave,row,col,length):
    ...
    Returns: nothing
    """
    ### FIXME:  Fill in this function.  It is very similar to the 
    ### hwall function.
    for i in range(length):
        cave[col][row+i] = STONE
    pass
...
def fill(cave, row, col, color):
    ...
    """
    if(row > len(cave)-1 or row < 0) or (col > len(cave[0])-1 or col<0) or(cave[row][col]== STONE):
        return
    else:                                
...
def explore(cave):
    """
    #FIXME a docstring is needed here
    """
    ###
    ### FIXME: Count the number of air pockets discovered
    ###
    chambers = 0                                     
    for row in range( len(cave)) :
        for col in range( len(cave[0])):
            if cave[row][col] == AIR:
                ...
                grid.get_next_color()
                chambers += 1                             
    return chambers  #FIXME should be the number of air pockets
